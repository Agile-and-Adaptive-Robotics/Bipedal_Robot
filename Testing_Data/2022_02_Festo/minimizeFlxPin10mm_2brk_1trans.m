% routed to the superseded single-transform evaluator (minimizeFlxPin2brk_1trans, from commit 3374847).
% STAGED 2026-09-08 by laptop zcode session: UNTESTED, never executed. Copy of minimizeFlxPin10mm_2brk.m
...
TRANSMODE = '1trans';    %frame method used by the evaluator: '2trans' (two-rotation, current) or '1trans' (yaw-only)
...
%% Calculate baseline
[a0, bpa0] = minimizeFlxPin2brk_1trans(0,Inf,Inf,[],USE_BRACKET2);   %no extra length, infinite bracket stiffness
baselineScores = a0;
...
for ii = 1:N
    ...
    f_all = minimizeFlxPin2brk_1trans(Xi0, Xi1, Xi2, [], USE_BRACKET2);   % returns nBPA x 3 [RMSE, FVU, MaxResidual]
...
k2 = sol_actual(2);
k3 = sol_actual(3);
[f, bpa] = minimizeFlxPin2brk_1trans(k1, k2, k3, [], USE_BRACKET2);  % [f: nBPA x 3], [bpa: full struct]
...
function ff = min1(x, trainIdx, kompare, useB2)
    ...
    try
        [f_all, ~] = minimizeFlxPin2brk_1trans(Xi0, Xi1, Xi2, trainIdx, useB2); % Nx3 matrix for training BPAs
...
function [c, ceq] = nonlinc(X, baseline, trainIdx)
    ...
    try
        ...
        Xi1 = 10^X(2);
        Xi2 = 10^X(3);
        [f_all, ~] = minimizeFlxPin2brk_1trans(Xi0, Xi1, Xi2, trainIdx);
