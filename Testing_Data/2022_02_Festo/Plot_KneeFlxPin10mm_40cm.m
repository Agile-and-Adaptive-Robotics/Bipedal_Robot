...
tendon = 0.035;
Pres = 623.43;      %average pressure (kPa) for 40.6 cm
restingLength = rest;
...
%% Tests 1
%Test 1 == sheet FlxTest10mm_5 from Results_table_10mm_pinned

%% Torque calculated from measurements
Angle{1} = [-113.5	-102	-94.208	-87.7	-80.5	-45	-112.5	-102.4	-92	-86	-77.694	-70.8	-65.129	-60.462	-50.769	-45.74	-40.717	-34.973	-29.229]';

Torque{1} = [-0.04910938	-0.834032441	-1.720007398	-2.90010369	-4.547465374	-18.31924036	-0.147496596	-0.776756248	-1.843707449	-2.922890876	-5.055953262	-7.505390813	-10.01429628	-12.1371208	-16.32807498	-18.00999441	-19.56235574	-20.20069067	-19.95109612]';

%% Calculate Torque by finding force from muscle contraction and distance
%from force line of action to muscle ICR.

InflatedLength{1} = [339	340	341	343	346	378	336	337	339	341	346	351	356	361	370	374	378	381	382]'/1000;

ICRtoMuscle{1} = [5	20	33	60	62	85	3	13	35	44	50	76	82	85	88	87	85	85	85]'/1000;


%load pressure where applicable
pres{1} = [624	623	624.365	624	623	622.8	625	624.5	624	622	622	625	624.365	623.607	622.85	621.336	622.85	623.607	622.8]';
...
Test = ["FlxTest10mm_5--10mm_pinned"];
...
hold off
title('Isometric Torque vs Knee Angle, 10 mm Flexor, 40.6 cm long')
xlabel('Knee angle, \circ')
