...
rest{1} = 0.436;        %resting length clamp to clamp, minus the barb
kmax{1} = 0.366;           %length at maximum contraction
tendon{1} = 0;            %no tendon condition
pres = 621.774;        %Average Pressure, kPa
Vas_Pam_43cm = MonoPamDataExplicit(Name, Location, CrossPoint, Dia, T, rest{1}, kmax{1}, tendon{1}, fitting, pres);
Theoretical = Vas_Pam_43cm.Torque(:,3);

rest{2} = 0.432;
kmax{2} = 0.356;
tendon{2} = 0.022;       %tendon
pres = 622.929;         %average pressure, kPa
Vas_Pam_43cm_tendon = MonoPamDataExplicit(Name, Location, CrossPoint, Dia, T, rest{2}, kmax{2}, tendon{2}, fitting, pres);
Theoretical_ten = Vas_Pam_43cm_tendon.Torque(:,3);

%% Tests done with CALT load cell.
%Test 1 == sheet ExtTest10mm_12 from Results_table_10mm_pinned_LoadCell   (no tendon)
%Test 2 == sheet ExtTest10mm_11 from Results_table_10mm_pinned_LoadCell   (tendon) 
...
Ma1 = Vas_Pam_43cm.MomentArm;                  %Calculated moment arm
G1 = hypot(Ma1(:,1),Ma1(:,2));         %Moment arm for z-axis torque
Ma2 = Vas_Pam_43cm_tendon.MomentArm;                  %Calculated moment arm
...
%Calculated strain
strain = Vas_Pam_43cm.Contraction;          %strain w/o tendon
strain_ten = Vas_Pam_43cm_tendon.Contraction;  %strain w/ tendon
...
MuscleLength = Vas_Pam_43cm.MuscleLength-2*fitting-tendon{1};
MuscleLength_ten = Vas_Pam_43cm_tendon.MuscleLength-2*fitting-tendon{2};
