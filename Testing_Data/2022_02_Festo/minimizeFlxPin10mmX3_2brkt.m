...
%Minimization scheme
% minimizeFlxPin10mmX3_2brkt.m — variant of minimizeFlxPin10mmX3.m (Ben 2026-09-15):
% calls the TWO-BRACKET evaluator minimizeFlxPinX3_2brkt; optional env lock
% FX3B_LOCK1 / FX3B_LOCK2 (N/m) pins Xi1/Xi2 so the GA searches only Xi0 and Xi3.
% The original driver minimizeFlxPin10mmX3.m is untouched.
...
%% Calculate baseline
[a0, bpa0] = minimizeFlxPinX3_2brkt(0,Inf,Inf,0);         %Get current goodness of fit measures with no extra length and infinite bracket stiffness
baselineScores = a0;
...
%% Problem bounds
lb = [0*100, log10(5e3), log10(5e3), 0];   %Xi3: unitless wrap-loss factor, linear [0,1]
ub = [0.020*100, log10(5e7), log10(5e7), 1];

%% FX3B lock (Ben 2026-09-15): pin Xi1/Xi2 to a mat's pair; the GA searches Xi0 and Xi3
FX3B_LOCK1 = getenv('FX3B_LOCK1');
FX3B_LOCK2 = getenv('FX3B_LOCK2');
if ~isempty(FX3B_LOCK1) && ~isempty(FX3B_LOCK2)
    fx3b1 = sscanf(FX3B_LOCK1, '%g');
    fx3b2 = sscanf(FX3B_LOCK2, '%g');
    lb = [lb(1), log10(fx3b1), log10(fx3b2), lb(4)];
    ub = [ub(1), log10(fx3b1), log10(fx3b2), ub(4)];
    fprintf('FX3B LOCK: Xi1=%.6g Xi2=%.6g locked; GA searches Xi0 [%.4g %.4g] cm, Xi3 [%.3g %.3g] unitless\n', ...
        fx3b1, fx3b2, lb(1), ub(1), lb(4), ub(4));
end
...
for i = 1:numBPA
    ...
    ind = (1:size(x2,1)).';                     %index by FRONT ROWS, not length(): a
    % single-member front returns x as 1x4 and length()=4 breaks the row assembly
    % (latent bug also in minimizeFlxPin10mmX3.m, fired 2026-09-15, fixed here)
...
x_actual = [results_sort(:,xCols(1))/100, 10.^results_sort(:,xCols(2)), 10.^results_sort(:,xCols(3)), results_sort(:,xCols(4))];   %Xi3 linear
...
for ii = 1:N
    ...
    f_all = minimizeFlxPinX3_2brkt(Xi0, Xi1, Xi2, Xi3);   % returns 4×3 [RMSE, FVU, MaxResidual] Assuming number of BPAs = 4.
...
% k3 = 3e3;
% k4 = 5e6;
[f, bpa] = minimizeFlxPinX3_2brkt(k1, k2, k3, k4);  % [f: 4x3], [bpa: full struct]
...
function ff = min1(x, trainIdx, kompare)
    ...
    Xi1 = 10^x(2);
    Xi2 = 10^x(3);
    Xi3 = x(4);   %Xi3: unitless wrap-loss factor, linear

    try
        [f_all, ~] = minimizeFlxPinX3_2brkt(Xi0, Xi1, Xi2, Xi3, trainIdx); % Nx3 matrix (e.g., 3x3 if 3 training BPAs)
...
function [c, ceq] = nonlinc(X, baseline, trainIdx)
    ...
    try
        ...
        [f_all, ~] = minimizeFlxPinX3_2brkt(Xi0, Xi1, Xi2, Xi3, trainIdx);
