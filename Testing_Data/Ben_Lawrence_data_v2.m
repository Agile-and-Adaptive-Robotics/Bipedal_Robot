...
Ben_test = [data11cm; data42cm; data45cm; data49cm; data52cm]; %Ben's testing data, in one matrix.
...
title('10mm normalized')

%addnoise to anchor 3 points of surface
err = 0.01;                     % This is 1% error around end points
sz = 100;                       %Size of anchor matrices
r110 = [1-err + 2*err * rand(sz,2), -err + 2*err * rand(sz,1)].*[1 620 1]; %Max relative strain is at max pressure, which means force is 0 (by definition)
r011 = [-err + 2*err * rand(sz,1), 1-err + 2*err * rand(sz,2)].*[1 620 1]; %Zero strain and maximum pressure create maximum force, by definition
r000 = -err + 2*err * rand(sz,3);                                           %No strain and no pressure means no force. This is resting length.   

test9 = [data13cm_test9(:,1) data13cm_test9(:,2) (data13cm_test9(:,3))/Fmax13;
...
numrows = length(test9)/100;
%test9 = imresize(test9,[numrows 3], 'nearest');
...
test10 = [data13cm_test10(:,1) data13cm_test10(:,2) (data13cm_test10(:,3))/Fmax13;
                data23cm_test10(:,1), data23cm_test10(:,2), (data23cm_test10(:,3))/Fmax23; 
                data27cm_test10(:,1) data27cm_test10(:,2) (data27cm_test10(:,3))/Fmax27; 
                data29cm_test10(:,1) data29cm_test10(:,2) (data29cm_test10(:,3))/Fmax29; 
                data30cm_test10(:,1) data30cm_test10(:,2) (data30cm_test10(:,3))/Fmax30]; 

numrows = length(test10)/100;
%test10 = imresize(test10,[numrows 3], 'nearest');

allData_test10 = [test10;
                Ben_test;
                r110; r011; r000];
...
figure
scatter3(X9, Y9norm, Z9)
xlabel('Relative Strain')
ylabel('Pressure (normalized)')
zlabel('Force (normalized)')
title('10mm, test 9, normalized')
