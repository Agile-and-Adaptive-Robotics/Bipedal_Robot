...
void sendValveAcknowledgement(char command)
{
  Serial.print("ACK,");
  ...
  while (Serial.available() > 0)
  {
    ...
    switch (command)
    {
      case 'V':
        ...
        setValves(true);
        sendValveAcknowledgement('V');
        break;

      case 'O':
        // Both valve-control outputs LOW
        setValves(false);
        sendValveAcknowledgement('O');
        break;
...
void setup()
{
  ...
  // Distinguish startup state from a response to a valve command
  sendBootState();
}
