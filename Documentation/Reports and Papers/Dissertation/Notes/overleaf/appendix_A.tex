...
The normalized isometric force surface of Equation~(\ref{eq:Fstar}) was fitted
by nonlinear least squares with least-absolute-residual robustness, and its
coefficients are reported in Table~\ref{tab:Fstar}. The code holds two
implementations of this surface: a closed-form evaluation of
Equation~(\ref{eq:app_fstar}) and a gridded interpolation of the manufacturer's
datasheet curves, with force clamped to zero for $\varepsilon^{*}>1$ or
$F^{*}<0$. The runtime torque evaluators used throughout this dissertation
evaluate the gridded interpolation.
% TODO(Ben): the closed-form file (Functions/f_festo.m) is the only place the
% Table tab:Fstar coefficients appear; the evaluators call Functions/festo4.m
% (FestoLookup.mat). Confirm which surface generated the reported Chapter 4
% numbers and reconcile with Methods eq (Fstar) before final submission.
...
only where the BPA is in tension; values below the compressive-strain cutoff
are discarded ($\varepsilon < -0.02$, or $\varepsilon < -0.03$ for the
\qty{20}{\mm} assemblies).

\section{Compliance Model Implementation}

The compliance-corrected muscle length entering Equation~(\ref{eq:Lm}) is
computed as $l_{m} = l_{LMT} - \chi_{0} - l_{ten} - 2\,l_{fitting}$. Equation
(\ref{eq:Lm}) states this length without the explicit tendon term, subsuming it
in the path length; the implementation keeps the two separate.
% TODO(Ben): reconcile eq (Lm)/eq (x3) in Methods with the implemented length. The bracket
...
bracketed root solve (Matlab's \texttt{fzero}) on $[0,\, l_{m}-l_{620}]$,
\begin{equation}
