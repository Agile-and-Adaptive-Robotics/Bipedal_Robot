...
This appendix documents the neuron and synapse models used by the synthetic nervous systems in this project, both in the current AnimatLab walker models and as they will be instantiated in SNS-Toolbox for the MuJoCo pipeline of Chapter~\ref{ch:futurework}. All parameter values below are read from the working bipedal walker model (\texttt{Biped\_2xCPG\_wSubs/walk new new tester\_Standalone.asim}).
...
These are the standard AnimatLab firing-rate/conductance models. The SNS-Toolbox \citep{nourse_sns_2023} implements the same classes of non-spiking and spiking neurons with conductance-based synapses, so the parameter set above transfers directly to the rhythm-generator, pattern-formation, and feedback circuits to be built for the MuJoCo pipeline (Chapter~\ref{ch:futurework}). The tonic stimulation tests of Section~\ref{sec:fw_cpg} correspond to constant $I_{\text{app}}$ injections into Equation~(\ref{eq:app_membrane}) at the motoneuron, pattern-formation, and rhythm-generator layers respectively.
