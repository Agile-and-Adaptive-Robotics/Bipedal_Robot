...
where $\hat{\mathbf{u}} = [u_{x}, u_{y}, u_{z}]^{T}$ is the BPA force direction expressed in the bracket frame. The bracket deflection vector, $\boldsymbol{\eta}_{br}$, was calculated from Hooke's law as

\begin{equation} \mathbf{F}' = \mathbf{K}_{br} \: \boldsymbol{\eta}_{br}
...
\begin{equation}
\boldsymbol{\eta}_{br} = \mathbf{K}_{br}^{-1} \: \mathbf{F}' = \mathbf{C}_{br} \: \mathbf{F}'
...
\begin{equation}
\boldsymbol{\eta}_{br} = \left\| \mathbf{F}' \right\| \mathbf{K}_{br}^{-1} \,\hat{\mathbf{u}}
...
\begin{equation}
\boldsymbol{\eta}_{br} = \begin{bmatrix} F'_{x,br}/\chi_{1} \\ F'_{y,br}/\chi_{2} \\ F'_{z,br}/\chi_{1} \end{bmatrix}
...
% from each bracket toward the other and satisfy F'_2=-F'_1. Each bracket has a
% local deflection vector eta and a clearly offset scalar projection delta along
% its force direction. The bracket-2 projection is shifted to the right of its
% coordinate axes. Tendon elongation is collinear and contiguous with the
% bracket-1 projection.
...
Although $\boldsymbol{\eta}_{br}$ is a three-dimensional deflection vector, the BPA force model depends on the scalar change in effective BPA length ($\delta$). Therefore, only the component of $\boldsymbol{\eta}_{br}$ along the BPA line of action directly changes the effective BPA length. This scalar bracket deflection is the projection of $\boldsymbol{\eta}_{br}$ onto $\hat{\mathbf{u}}$:

\begin{equation}
{\delta}_{br} = \hat{\mathbf{u}}^{T} \boldsymbol{\eta}_{br}
...
In parallel with the knee test stands, the author, together with McNeal and Williams, developed a family of hardware testbeds for the balance problem that a walking humanoid must eventually solve: a robotic inverted pendulum actuated by BPAs at the ankle and standing on a motorized oscillating balance platform \citep{bolen_balance_2024, bolen_selfbalancing_2024, bolen_selfbalancing_srs_2024}. The testbeds deliberately reduce the robot to a single actuated joint and a single sensory question --- which measurement of body orientation, fed through which feedback law, will hold the leg upright on a moving support --- so that vestibular and proprioceptive feedback strategies could be exercised in hardware before their adoption in the full humanoid, whose lateral stabilization is future work (Chapter~\ref{ch:futurework}). Three generations were built (Figure~\ref{fig:balance_inverted_pendulum}).
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.60\textwidth,height=0.62\textheight,keepaspectratio]{figs/Balance/inverted_pendulum_photo.jpg}
