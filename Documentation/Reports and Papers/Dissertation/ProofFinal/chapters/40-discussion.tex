...
\section{Tendon Transmission Trade-offs in the Reverse-Pulley Arrangement}\label{sec:disc_transmission}

The reverse-pulley transmission of the inverted-pendulum testbeds (Section~\ref{sec:balance_testbeds}) \citep{bolen_balance_2024, bolen_selfbalancing_2024, bolen_selfbalancing_srs_2024} illustrates a design trade that recurs wherever a BPA drives a joint through a tendon. The four-wheel block-and-tackle buys joint travel with muscle force: reeving the paired BPAs' tendon through the pulley train mounted on the foot multiplies the tendon travel delivered at the joint at a proportional cost in force transmitted to the tibia. For an actuator whose limited contraction forces long routings --- the burden visible in the extensor redesign (Section~\ref{sec:ongoing}) and taken up for the humanoid in Chapter~\ref{ch:futurework} --- that exchange can be exactly what a design needs. The arrangement also collects its price structurally: because the pulley block mounts on the foot, the tendon tension reacts into the structure that carries the pulleys rather than passing cleanly along the tendon line, and this reaction force deformed the printed foot significantly at the pulley mounting location. Transmission hardware, in other words, is not kinematically free; it loads and deflects the segments it rides on.

The arrangement also complicates the geometry on which the produced torque depends. With a fixed insertion, the force unit direction used in the moment-arm calculation follows from static attachment geometry; with the tendon wrapping over pulleys that move with the joint, the wrapping point that fixes that direction migrates as the joint angle changes. This migration can increase the effective moment arm over part of the travel --- part of the arrangement's appeal --- but it makes the kinematics considerably more daunting: the moment arm becomes a function of joint angle that must itself be modeled, and errors in the assumed wrap geometry propagate into torque just as the moment-arm sensitivity discussed in Section~\ref{sec:disc_error} predicts.

Two solutions present themselves. The first is to track the moving wrap point in the model, accepting the more demanding kinematics in exchange for the pulley train's mechanical simplicity and its moment-arm advantage. The second is to eliminate the moving wrap with a Bowden cable --- the tendon running in a sheath anchored to both bodies --- which fixes the force direction on each body but demands packaging space for a bracket and a barrel adjuster on both bodies, trading kinematic simplicity for routing hardware. Either solution can serve; the choice turns on the goals of the researchers, on whether model fidelity or transmission simplicity is worth more for the joint being designed. Tracking a moving wrap point in the route model is not new ground for this dissertation's optimization work --- the biomimetic flexor route already carries one explicitly (Section~\ref{sec:optimization}; Appendix~\ref{app:model_geometry}) --- but no pulley-aware model of this arrangement has been built, and the arrangement remains a candidate for the humanoid joints whose optimized routes demand impractically long actuators (Chapter~\ref{ch:futurework}).
...
This chain was developed deliberately so that each stage can feed the next. The force model supplies the actuator terms for the torque model; the torque model supplies the plant model for designing joint controllers; and the validated biomimetic knee demonstrates that the actuator architecture can reproduce human joint-level force capabilities. The neural layer is no longer only a proposal: the OpenSim-to-MuJoCo conversion, BPA coupling interface, SNS-Toolbox spinal architecture, sensory channels, and staged tuning workflow described in Sections~\ref{sec:preliminary_sim_methods}--\ref{sec:preliminary_sim_results} provide a working framework for neuromechanical experiments. The AARL's prior AnimatLab walker (Section~\ref{sec:prior_walker}) explains why that framework emphasizes systematic verification: its hand-tuned two-layer CPG produced gait-like suspended stepping, but its afferent feedback did not generalize to weight-bearing motion. The remaining work is therefore not to invent a neural layer, but to quantify its closed-loop behavior, improve robustness on the ground, and transfer the verified controller to hardware, as described in Chapter~\ref{ch:futurework}.
