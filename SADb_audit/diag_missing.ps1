...
$elig = Get-Content "D:\Github\Bipedal_Robot\SADb_audit\personal_SAD_attachment_purge_eligible.csv" -Encoding UTF8 | ConvertFrom-Csv
...
$per = Get-Content "D:\Github\Bipedal_Robot\SADb_audit\personal_SAD_items_slim.csv" -Encoding UTF8 | ConvertFrom-Csv
$perBy = @{}; foreach ($p in $per) { $perBy[$p.key] = $p }
$bio = Get-Content "D:\Github\Bipedal_Robot\SADb_audit\group_Biology_items_slim.csv" -Encoding UTF8 | ConvertFrom-Csv
