# SADb Audit — Sensory Afferent Database three-way reconciliation
...
6. Append the batch to `SADb_audit\curation_log.csv` (date, batch #, record
...
**Personal SADb ⊆ AARL group, essentially exactly:**
...
## The AARL "SADb" corpus = the Biology folder subtree (Ben, 2026-09-10)

SADb material is NOT one collection in AARL — it lives under the **Biology**
...
- Core: **Sensory Feedback** (`N9R9QB36`) with **532 items** ≈ the personal SADb set
...
- Personal SADb vs Biology subtree: **537/538 found** (miss = the empty
  placeholder `DLHNVKD7`) → personal SADb ⊆ AARL Biology, confirmed
...
- **Personal SADb (538) → Airtable:** **95/538 present → 443 missing**
...
3. **Full personal-SADb import DONE**: 390 remaining gap rows → 7 dropped as
...
- Approved: add 3 missing papers to Zotero, fix Airtable typos + years, DOI field
  (done, see below), and the personal-SADb attachment purge with two-part criterion:
...
- **3 papers ADDED to personal SADb (RBG8ZVYP)**, 200/3-successful/0-failed:
  Hultborn 1971 = `6MNCI8I6`, Zill 2015 = `8H7VACP4`, Dietz 2003 = `928TAKM7`.
  Script: `zotero_web_add3.ps1` (payload `add3_items.json`).
- **Attachment purge EXECUTED: 107 attachments deleted from 80 personal-SADb items**
...
(his personal-SADb gap list; first 50 clean items). Fields: Name/Primary Author/
...
via web API (see above). Personal SADb ∩ Airtable = 96/538 (post-fix).
...
- **Research Rabbit:** Ben's RR collection WAS observed live (Chrome window; selected
  ...
  Zhang 2022=already in SADb, Harrison 2023=judged out of scope). Deep click-through died
...
## CURATION STATE (2026-09-18, EB475WS4) — batch 6 DONE; corpus = 943; offline explorer built

- **Batch 6 + 7 complete + verified** (2026-09-18/20): rows 41–51 and 52–61 of the
  queue → **16 papers curated** (echo-verified by re-pull), **4 logged no-text** —
  Thiry 2020 (D8PKQH5X), Prochazka 2017 (BKERX7AM), Yakovenko 2001 (L8WLNNF3),
  Lewinger 2006 (WJ4ZTW5P): Elsevier/CRC chapters and a DOI-less robot paper with
  no abstract anywhere; left bare per the no-grounding-no-note rule, Ben library
  pass needed. 7 twin records created (Models: Fujiki 2018 recxgsj4gWFoEA3Jz,
  Ekeberg 2004 receLzSMQA9RYo4t3, Bondy 2016 recVUX3okwyChn7ew; Review Papers:
  Côté 2018 rectKTUSAaUJIw5tG, Prochazka and Ellaway 2012 reckH8W6X4AxkLFeH,
  Ziskind-Conhaim and Hochman 2017 reckp7LCl56L0mh0a, Duysens and Forner-Cordero
  2018 recvisSCDWFyYXAfJ). Year fix: Bouyer/Rossignol Part II → 2003.
  **Papers = 943; 459 with notes; 484 bare. Queue progress 66/383 (batches 1–7).
  Flags for Ben in the log:** hip-entrainment Feedback record proposal (Andersson &
  Grillner 1983), Drosophila load-feedback campaniform question (Nourse 2019),
  Duysens 2018/2019 near-twin review names.
- **PDF hunt (2026-09-20, pdf_hunt2.py + pdf_hunt_retry.py): the ENTIRE DOI-bearing
  corpus (886) is now hunted, not just the old 456 — 44 more PDFs attached (274→318;
  the 48 apparent attach failures were a fields[]-GET bug, all succeeded on retry).
  THE definitive no-PDF hunt list is `author_fix\remaining_no_pdf.csv` — one file,
  columns doi/record_id/title/hunt_status, regenerated by `rebuild_no_pdf_list.py`
  (581 records: 488 no OA copy anywhere, 88 candidate URLs that fail %PDF or are
  bot-blocked, rest stragglers/new). The Sept-15 intermediate lists
  (paperhunt_status.csv, pdfs_missing_no_zotero.csv, pdfs_paywalled.csv,
  pdfs_to_attach.json, upload/fileio/tunnel status logs, the dated pass-2 files,
  pdf_match_pairs.csv) were DELETED 2026-09-20 — do not resurrect them;
  `pdfs_from_zotero.csv` stays (inventory of where Zotero PDF copies live, for
  drag-drop). Records without DOI can't be URL-hunted — 44 such.
- **Reconciliation finding**: the task5 auto-curation pass did NOT curate the
  rest-import queue — `reconcile_queue.py` (reusable) matches queue rows against the
  live export by DOI/title. Batch 7 = rows 52–60; grounding for rows 51–70 already
