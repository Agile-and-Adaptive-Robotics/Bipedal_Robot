...
corpus_dois = set()
with open(r"C:\Users\Ben\Documents\GitHub\Bipedal_Robot\SADb_audit\personal_SAD_items_slim.csv", encoding="utf-8", errors="replace") as f:
    for row in csv.DictReader(f):
        for v in row.values():
            if v and re.match(r"^10\.\d{4}", v.strip()):
                corpus_dois.add(v.strip().lower())
with open(r"C:\Users\Ben\Documents\GitHub\Bipedal_Robot\SADb_audit\airtable_rest_import_clean.csv", encoding="utf-8", errors="replace") as f:
    for row in csv.DictReader(f):
