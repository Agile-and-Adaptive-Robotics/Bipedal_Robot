...
deduped = []
for r in records:
    key = (r["fields"].get("DOI", ""), r["fields"].get("Name", ""))
    if key in seen:
        continue
    seen.add(key)
    deduped.append(r)
records = deduped
print("deduped:", len(records))

# create in batches of 40
created = []
fail_count = 0
for i in range(0, len(records), 40):
    ...
    try:
        ...
        print(f"batch {i//40+1}: created {len(resp.get('records', []))}")
    except urllib.error.HTTPError as e:
        print(f"batch {i//40+1} FAIL: {e.code} {e.read().decode()[:200]}")
        fail_count += len(chunk)
    except Exception as e:
        print(f"batch {i//40+1} FAIL: {e}")
        fail_count += len(chunk)
    time.sleep(1)
