# Corpus discovery tool plan — a private "Research Rabbit" for the SADb corpus

Written 2026-09-12 (laptop session). Goal: RR-style discovery (similar work + newest
articles + bubble graph) over OUR corpus (Airtable Papers 532 / personal Zotero SADb 538),
tied to the SADb curation tags instead of locked in someone else's web app.

## Why not just keep using Research Rabbit
RR is good for interactive triage, but: its collections are separate from SADb (no
...
`SADb_audit\`. Output: coverage report (papers OpenAlex can't find; missing DOIs to backfill).
...
### Phase 3 — Interactive HTML map tied to SADb tags (scriptable route)
`corpus_map.py` → pyvis/networkx HTML in `SADb_audit\maps\`:
...
3. Output a **Markdown digest** (`SADb_audit\digests\YYYY-WW.md`): ranked table
