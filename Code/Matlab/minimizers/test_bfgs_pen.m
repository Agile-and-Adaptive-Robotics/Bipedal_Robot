...
f = @(x) [0.5*x'*x;(1+x(1).^2-x(2)).^2;(-2*x(1)-x(2)).^2];
...
search_options = {'backtrack',1e-4,0.5,15,0.8,.01};
plot_options = [1,3];
print_options = 1;
parallelize = false;
...
x_final = bnd_con_pen_bfgs(f,x0,bnds,cons,conv_crit,direction_options,search_options,plot_options,print_options)
