function [ x_final, f_final ] = bnd_bfgs(f, x0, bounds, linear_constraints,...
...
%     mu = 10^(ceil(log10(abs(f_orig))) - oom_diff);
    n = length(x0);
    
    A_bnds = [];
    b_bnds = [];
    if isempty(bounds)
        warning('If no bounds are needed, use bfgs.m, which is unbounded.')
        bnds = [];
    else
    ...
    penalty_weight = 0; %If we want penalties, use bnd_con_pen_bfgs()
...
%             [X,Y] = meshgrid(linspace(bounds(1,1),bounds(1,2),20),linspace(bounds(2,1),bounds(2,2),20));
            [X,Y] = meshgrid(linspace(xmin-0.25*xrange,xmax+.25*xrange,20),...
                linspace(ymin-0.25*yrange,ymax+0.25*yrange,20));
            xtemp = X(:);
            ytemp = Y(:);
            for j=1:400
