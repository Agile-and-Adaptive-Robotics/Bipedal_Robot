...
if pred0.routeInfo.releaseFound
    fprintf('interpolated release angle  = %+.6f deg\n', ...
        pred0.routeInfo.releaseAngleD)
else
    fprintf('interpolated release angle  = NONE IN MODELED RANGE\n')
end
...
yline(0, ':k', 'Minimum strain', 'HandleVisibility', 'off')
yline(ctx.KMAX, ':', 'KMAX', 'HandleVisibility', 'off')
box off
grid off
xlabel('\theta_k, deg')
ylabel('Strain')
