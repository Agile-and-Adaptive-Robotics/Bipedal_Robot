...
if pred0.BPAcount == 2
    fprintf('\nBPA 2 route (same-side p1, mirrored pEnd):\n')
    fprintf('p1{2}, femur                = [%.6f %.6f %.6f] m\n', pred0.p1B)
    fprintf('pEnd{2}, t1                 = [%.6f %.6f %.6f] m\n', pred0.p2B)
    fprintf('BPA 2 wrap active at +10    = %d\n', ...
        pred0.routeInfo2.active(idxFullExtension))
    fprintf('BPA 2 wrap release found    = %d\n', ...
        pred0.routeInfo2.releaseFound)
    if pred0.routeInfo2.releaseFound
        fprintf('BPA 2 first inactive wrap   = %+.6f deg\n', ...
            pred0.routeInfo2.releaseAngleD)
    end
end
...
for k = 1:numel(poseIndex)
    ...
    PB = pred0.Location2(:,:,ii);
    for j = ctx.CrossPoint:size(PB,1)
        PB(j,:) = RowVecTrans(ctx.T_Pam(:,:,ii), PB(j,:));
    end
    ...
    hRouteB = plot(ax, PB(:,1), PB(:,2), '--', ...
        'Color', c{6}, 'LineWidth', plt.lineW, ...
        'MarkerEdgeColor', 'none', 'DisplayName', 'BPA 2 route');
    hP1 = scatter(ax, P(1,1), P(1,2), plt.scattersz, ...
        'MarkerFaceColor', c{2}, 'MarkerEdgeColor', 'none', ...
        'DisplayName', 'p1');
    hP1B = scatter(ax, PB(1,1), PB(1,2), plt.scattersz, ...
        'MarkerFaceColor', c{4}, 'MarkerEdgeColor', 'none', ...
        'DisplayName', 'p1 (BPA 2)');
    ...
    if k == numel(poseIndex)
        lgd = legend(ax, [hRoute, hRouteB, hP1, hP1B, hWrap, hEnd], ...
            {'BPA route', 'BPA 2 route', 'p1', 'p1 (BPA 2)', 'pWrap', 'pEnd'}, ...
...
fprintf('routes checked             = %d (binding: BPA %d)\n', ...
    collision0.routeCount, collision0.bindingRoute)
