...
%BUILDKNEEFLEXORCONTEXT20MM Assemble fixed inputs for one flexor run.
% ctx is short for "context." It is an ordinary struct used to pass the
% transforms, geometry, BPA constants, objective settings, initial design,
% and bounds together to the predictor, objective, and constraints.
% Values that vary during optimization belong in x, not ctx.
...
ctx.fitting    = 0.021;
% Radius source used by predictKneeFlexor20mm:
%   "bpaR"   -> call bpaR(strain_p,Dia,KMAX), producing one radius per
%               knee angle. predictKneeFlexor20mm repeatedly rebuilds the
%               route because contraction changes radius and radius changes
%               the moving wrap point.
%   "scalar" -> do not call bpaR. Use the hand-set geo.bpaRb and geo.bpaRs
%               values from buildGeoExclusion. Each may be one scalar or an
%               N-by-1 array. Set this line to "scalar" for fixed radii.
% buildKneeFlexorRoute20mm uses bpaRb to calculate pWrapT1 at every angle
% and stores the transformed active point in Location(2,:,i). bpaRs is used
% for straight-segment collision clearance.
...
% Original one-BPA, two-point route used for plots and the separate x/y
% off-axis limits. predictOriginalKneeFlexor20mm explicitly passes
% BPAcount = 1 and Xi3 = 0. The optimized route alone uses two BPAs.
