...
%PREDICTORIGINALKNEEFLEXOR20MM Original one-BPA, two-point route.
% This comparison deliberately uses one BPA and Xi3 = 0.  The optimized
% prediction uses two actual mirrored routes through MonoPam_mult.
...
try
    bpa = MonoPamDataExplicit_balanceX3( ...
        ...
        ctx.Xi1, ...
        ctx.Xi2, ...
        0, ...                         % original comparator has no Xi3 loss
        ctx.originalWraps, ...
        ctx.phiD, ...
        1, ...                         % original route contains one BPA
        bendMeasure);
catch ME
    pred.ok = false;
