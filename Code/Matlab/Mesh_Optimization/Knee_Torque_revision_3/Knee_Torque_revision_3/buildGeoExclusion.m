...
% bpaRadiusMode is the text selected in buildKneeFlexorContext20mm:
%   "scalar" keeps the manual bpaRb/bpaRs values below;
%   "bpaR" tells predictKneeFlexor20mm to replace them with arrays calculated
%   from bpaR at the candidate's physical contraction.
%
% p2 = [px py pz]
%
% Case 1:
...
% Hand-set mode: set ctx.bpaRadiusMode = "scalar", then edit these values.
% Each may be a scalar (constant over the motion) or N-by-1 array.
geo.bpaRb = 0.016;        % wrap-point standoff
geo.bpaRs = 0.013;        % straight-segment collision radius

% bpaR mode: predRadius means the N-by-1 physical radius calculated as
% bpaR(strain_p,Dia,KMAX) inside predictKneeFlexor20mm. It is not a separate
% user input in that mode. The two radii used by the route/collision code are:
%   bpaRb = predRadius + bpaRbOffset
%   bpaRs = predRadius + bpaRsOffset
% For your example, set these to +0.002 and -0.001 respectively.
% Zero preserves the current physical-radius calculation.
geo.bpaRbOffset = 0;
geo.bpaRsOffset = 0;
...
% wRap*wrap angle is an arc-length estimate used only for Xi3 bend loss.
% It is not a torus arc added to the Location centerline or MuscleLength.
geo.wRap  = 0.025;
