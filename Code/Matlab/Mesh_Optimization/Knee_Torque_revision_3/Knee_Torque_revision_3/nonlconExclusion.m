function [c, ceq, info] = nonlconExclusion(x, geo, ctx, idxP2, predRadius)
%NONLCONEXCLUSION Attachment and inflated-BPA collision constraints.
% predRadius is an optional fifth input retained for efficient optimizer
% calls. It may be: [] (use geo radii), a scalar/N-vector base radius, or
% the complete pred struct already calculated by predictKneeFlexor20mm.
...
% Fifth input:
%   omitted: follow ctx.geo.bpaRadiusMode (existing optimizer calls);
%   []: use geo.bpaRb and geo.bpaRs directly;
%   scalar or N-vector: base radius, plus the offsets in buildGeoExclusion;
%   prediction struct: reuse the objective's solved route (legacy calls).
if nargin < 5
    if string(geo.bpaRadiusMode) == "bpaR"
        ctx.geo = geo;
        predRadius = predictKneeFlexor20mm(x, ctx);
    else
        predRadius = [];
    end
end
...
if isstruct(predRadius)
    ...
    if ~isempty(predRadius)
        ...
        if ~isfield(geo,'bpaRsOffset'), geo.bpaRsOffset = 0; end
        validateattributes(predRadius, {'numeric'}, ...
            {'real','vector','finite','positive'});
        geo.bpaRb = predRadius(:) + geo.bpaRbOffset;
...
bpaRbByFrame = routeInfo.bpaRb;
bpaRsByFrame = routeInfo.bpaRs;
bpaRbCollision = bpaRbByFrame(idxCollision);
bpaRsCollision = bpaRsByFrame(idxCollision);

info.bpaRadiusMode = string(geo.bpaRadiusMode);
info.bpaRb = bpaRbCollision;       % pWrap standoff at collision pose
info.bpaRadius = bpaRbCollision;   % legacy diagnostic alias
info.bpaRs = bpaRsCollision;       % collision radius at collision pose
info.bpaRbByFrame = bpaRbByFrame;
info.bpaRsByFrame = bpaRsByFrame;
...
% In scalar mode, bpaRs is the hand-estimated compressed collision radius.
% In bpaR mode, bpaRs is the physical outer radius at this knee pose.
tibiaRadius = geo.tendonRadius*ones(size(pathPosition));
tibiaRadius(isBPA) = bpaRsCollision;
femurRadius = geo.tendonRadius*ones(size(pathPosition));
femurRadius(isBPA) = bpaRsCollision;
...
% The femur uses the same pose-specific physical radius as the tibia.
...
info.tibiaBPAMinRadius = bpaRsCollision;
info.femurBPANominalRadius = bpaRsCollision;
