...
% pose-by-pose arrays, including separate offsets from bpaR.
%
...
N = ctx.N;
% Normalize once here; nonlconExclusion reuses info.bpaRb/info.bpaRs.
bpaRb = ctx.geo.bpaRb(:);
bpaRs = ctx.geo.bpaRs(:);
if isscalar(bpaRb), bpaRb = repmat(bpaRb, N, 1); end
if isscalar(bpaRs), bpaRs = repmat(bpaRs, N, 1); end
validateattributes(bpaRb, {'numeric'}, {'real','finite','positive','numel',N});
validateattributes(bpaRs, {'numeric'}, {'real','finite','positive','numel',N});
...
% pWrap below is the moving wrap point, as an N-by-3 array in the t1 frame.
...
for i = 1:N
    ...
    if active(i)
        Location(:,:,i) = [p1; pWrapICR; pEndICR]; % row 2 = transformed moving pWrap

        % wRap*angle estimates arc length only for Xi3 bend loss. The
        % musculotendon centerline remains the straight p1-pWrap-pEnd
        % segments; this is not a torus-contact arc-length calculation.
