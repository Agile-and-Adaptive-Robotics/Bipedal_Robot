...
paramCmd = strjoin({ ...
    ...
    '% --- CPG ---' ...
    'drive  = 7.5;       % nA tonic drive to both RG neurons' ...
...
snsInst(mdl, 'SNS_Library/NonSpikingNeuron', 'Adp_flex', [420 330 520 430], ...
    'Vrest', '-52', 'Gm', '0.05', 'Cm', '30', 'Thr', '-45', 'Slope', '1');
% mutual inhibition between the RGs (half-center)
snsInst(mdl, 'SNS_Library/NonSpikingSynapse', 'inh_ext_on_flex', [330 240 400 300], ...
    'gmax', '0.35', 'Esyn', '-72', 'ThrPre', '-45', 'SlopePre', '0.5');
...
snsInst(mdl, 'SNS_Library/NonSpikingSynapse', 'exc_ext_adp', [340 70 410 120], ...
    'gmax', '0.06', 'Esyn', '0', 'ThrPre', '-45', 'SlopePre', '0.5');
snsInst(mdl, 'SNS_Library/NonSpikingSynapse', 'exc_flex_adp', [340 340 410 390], ...
    'gmax', '0.06', 'Esyn', '0', 'ThrPre', '-45', 'SlopePre', '0.5');
% adaptive inhibition Adp -> own RG
snsInst(mdl, 'SNS_Library/NonSpikingSynapse', 'adp_inh_ext', [300 20 370 55], ...
    'gmax', '0.30', 'Esyn', '-72', 'ThrPre', '-45', 'SlopePre', '0.5');
snsInst(mdl, 'SNS_Library/NonSpikingSynapse', 'adp_inh_flex', [300 445 370 480], ...
    'gmax', '0.30', 'Esyn', '-72', 'ThrPre', '-45', 'SlopePre', '0.5');

% input current sums (drive + all synapse currents) -> RG neurons
...
add_block('simulink/Sources/Constant', [mdl '/drive_c'], 'Value', 'drive', 'Position', [40 110 70 140]);
add_line(mdl, 'drive_c/1', 'sumRG_ext/1', 'autorouting', 'on');
add_line(mdl, 'drive_c/1', 'sumRG_flex/1', 'autorouting', 'on');
...
% Adp V -> adaptive inhibition (pre side)
add_line(mdl, 'Adp_ext/1', 'adp_inh_ext/1', 'autorouting', 'on');
add_line(mdl, 'Adp_flex/1', 'adp_inh_flex/1', 'autorouting', 'on');
% post sides: mutual inhibition + adaptive inhibition into RG current sums
add_line(mdl, 'inh_ext_on_flex/2', 'RG_flex/1', 'autorouting', 'on');
add_line(mdl, 'inh_flex_on_ext/2', 'RG_ext/1', 'autorouting', 'on');
add_line(mdl, 'adp_inh_ext/1', 'sumRG_ext/2', 'autorouting', 'on');
add_line(mdl, 'adp_inh_flex/1', 'sumRG_flex/2', 'autorouting', 'on');
% slow excitation onto Adp neurons (post sides)
add_line(mdl, 'exc_ext_adp/2', 'Adp_ext/1', 'autorouting', 'on');
add_line(mdl, 'exc_flex_adp/2', 'Adp_flex/1', 'autorouting', 'on');
