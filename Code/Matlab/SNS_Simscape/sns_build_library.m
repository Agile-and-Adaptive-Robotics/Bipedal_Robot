...
% primary code; block tints follow Animatlab / journal-figure conventions
% (2026-09-10 restyle): pale-yellow neuron bodies, green = excitatory,
% red = inhibitory, salmon/red fusiform = muscle, green-gray = afferent.
% THICK STROKES: Simulink mask icons cannot set LineWidth, so outlines are
% drawn as filled shapes (black outer shape + slightly smaller filled inner
% shape) which renders as a heavy, journal-weight outline at any zoom.
...
function s = neuronIconCode(lbl)
    % Open circle with HEAVY outline (drawn as black outer ring + filled
    % inner circle; mask icons cannot set LineWidth). Pale-yellow body,
    % Animatlab-style. Ring thickness ~0.11 in icon units.
    s = strjoin({ ...
        't_ = linspace(0, 2*pi, 73);' ...
        'patch(0.92*cos(t_), 0.92*sin(t_), [0 0 0]);' ...
        'patch(0.81*cos(t_), 0.81*sin(t_), [0.99 0.96 0.81]);' ...
        ['disp(''' lbl ''');'] ...
        }, newline);
end

function s = lifIconCode()
    % Heavy-outline circle with a spike glyph inside (distinguishes spiking cell).
    s = strjoin({ ...
        't_ = linspace(0, 2*pi, 73);' ...
        'patch(0.92*cos(t_), 0.92*sin(t_), [0 0 0]);' ...
        'patch(0.81*cos(t_), 0.81*sin(t_), [0.99 0.96 0.81]);' ...
        'color(''black'');' ...
        'plot([-0.50 -0.22 0.0 0.22 0.50], [-0.12 -0.12 0.40 -0.12 -0.12]);' ...
        'plot([-0.50 -0.22 0.0 0.22 0.50]+0.012, [-0.12 -0.12 0.40 -0.12 -0.12]);' ...
        'plot([-0.50 -0.22 0.0 0.22 0.50]-0.012, [-0.12 -0.12 0.40 -0.12 -0.12]);' ...
        }, newline);
end

function s = afferentIconCode(lbl)
    % Heavy-outline circle, light green-gray tint (sensory afferent).
    s = strjoin({ ...
        't_ = linspace(0, 2*pi, 73);' ...
        'patch(0.92*cos(t_), 0.92*sin(t_), [0 0 0]);' ...
        'patch(0.81*cos(t_), 0.81*sin(t_), [0.85 0.93 0.85]);' ...
        ['disp(''' lbl ''');'] ...
        }, newline);
end

function s = muscleIconCode(lbl)
    % Fusiform (spindle-shaped) muscle with heavy outline; salmon-red body,
    % journal-figure muscle convention (was light green).
    s = strjoin({ ...
        't_ = linspace(0, 2*pi, 73);' ...
        'patch(0.92*cos(t_), 0.42*sin(t_), [0 0 0]);' ...
        'patch(0.81*cos(t_), 0.33*sin(t_), [0.94 0.76 0.74]);' ...
        ['disp(''' lbl ''');'] ...
        }, newline);
end

function s = synapseIconCode()
    % Rybak-style CONNECTION icon (restyle 2026-09-20, per Ben): the synapse
    % reads as an AXON passing through the block — heavy horizontal bar,
    % presynaptic side LEFT (Vpre port) — terminating in the postsynaptic
    % marker at the RIGHT edge (Isyn out), so a chained
    % neuron -> synapse -> neuron signal path renders as one continuous
    % neural connection with its terminal:
    ...
    %                               presynaptic side (Ben 2026-09-09), base
    %                               at the postsynaptic membrane
    %   unknown expression -> gray backdrop + "E?" text
    % The lower thin gray bar is the Vpost (postsynaptic voltage) sense wire.
    % Backdrop tints (light red / light green) kept as the redundant
    % CVD-safe cue. Heavy strokes = filled shapes (mask icons cannot set
    % LineWidth).
    s = strjoin({ ...
        'es_ = NaN;' ...
        'try' ...
        ...
        '    patch([-1 0.72 0.72 -1], [0.25 0.25 0.35 0.35], [0 0 0]);' ...
        '    t_ = linspace(0, 2*pi, 73);' ...
        '    patch(0.17*cos(t_) + 0.78, 0.17*sin(t_) + 0.30, [0 0 0]);' ...
        'else' ...
        '    patch([-1 1 1 -1], [-1 -1 1 1], [0.85 0.94 0.86]);' ...
        '    patch([-1 0.42 0.42 -1], [0.25 0.25 0.35 0.35], [0 0 0]);' ...
        '    patch([0.42 0.97 0.97], [0.30 0.66 -0.06], [0 0 0]);' ...
        '    patch([0.53 0.85 0.85], [0.30 0.57 0.03], [1 1 1]);' ...
        'end' ...
        'patch([-1 0.72 0.72 -1], [-0.53 -0.53 -0.47 -0.47], [0.35 0.35 0.35]);' ...
        }, newline);
end
