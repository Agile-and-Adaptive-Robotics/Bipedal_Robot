...
Location3 = [-0.111, -0.114, 0.049;
            0.007, -0.384, -0.027];
CrossPoint = 2;
Add_Mag3 = MonoMuscleData(Name, Location3, CrossPoint, MIF, TSL, Pennation, OFL, T);
...
Location = [-0.073, -0.117, 0.025;
            -0.004, -0.121, 0.034];
Add_Mag_Pam1 = MonoPamData(Name, Location, CrossPoint, Dia, T);

Name = 'Adductor Magnus 2';
Location2 = [-0.083, -0.119, 0.031;
            0.005, -0.229, 0.023];
Add_Mag_Pam2 = MonoPamData(Name, Location2, CrossPoint, Dia, T);

Name = 'Adductor Magnus 3';
Location3 = [-0.111, -0.114, 0.049;
            0.007, -0.384, -0.027];
Add_Mag_Pam3 = MonoPamData(Name, Location3, CrossPoint, Dia, T);
...
PAMTorque = Add_Mag_Pam1.Torque + Add_Mag_Pam2.Torque + Add_Mag_Pam3.Torque;
...
for i = 1:size(Pelvis, 1)
    for ii = 1:size(Femur, 1)
        Location1(1, :) = Pelvis(i, :);
        Location1(2, :) = Femur(ii, :);
        Add_Mag_Pam1.Location = Location1;
        Torque1 = Add_Mag_Pam1.Torque;
        
        for iii = 1:size(Pelvis, 1)
            for iiii = 1:size(Femur, 1)
                Location2(1, :) = Pelvis(iii, :);
                Location2(2, :) = Femur(iiii, :);
                Add_Mag_Pam2.Location = Location;
                Torque2 = Add_Mag_Pam2.Torque;
                
                for iiiii = 1:size(Pelvis, 1)
                    for iiiiii = 1:size(Femur, 1)
                        clc
                        fprintf('%d \t of %d \n', iC, size(Pelvis, 1)^3*size(Femur, 1)^3)

                        Location3(1, :) = Pelvis(iiii, :);
                        Location3(2, :) = Femur(iiiiii, :);

                        Add_Mag_Pam3.Location = Location3;

                        PAMTorque = Torque1 + Torque2 + Add_Mag_Pam3.Torque;
                        ...
                        if C < CMaxPrev
                            if isequal(Add_Mag_Pam.LengthCheck, 'Usable')
                                if isequal(Add_Mag_Pam2.LengthCheck, 'Usable')
                                    if isequal(Add_Mag_Pam3.LengthCheck, 'Usable')
                                        Tracker = [i, ii; iii, iiii; iiiii, iiiiii];
                                        CMaxPrev = C;
                                    end
                                end
                            end
