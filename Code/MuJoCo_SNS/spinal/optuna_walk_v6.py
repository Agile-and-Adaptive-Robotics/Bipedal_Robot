"""v6 study (ground_walk_v6_kneext): v5 + swing-knee quad suppression.
...
Seeded with the v5 winner (best_walk_params_v5.json multipliers + the new
gain at 0). Writes best_walk_params_v6.json (runner --best6 loads it) and
...
STUDY = "ground_walk_v6_kneext"
CSV = HERE / "v6_results.csv"
BEST6 = HERE / "best_walk_params_v6.json"
...
def full22_capture(study: optuna.Study, trial_no: int, p: dict):
    ...
    npz = HERE / f"v6_best_trial{trial_no}.npz"
...
def main(argv):
    ...
    if len(study.trials) == 0:
        ...
        try:
            prev = json.loads(
                (HERE / "best_walk_params_v5.json").read_text("utf-8"))
            mul = prev.get("multipliers", {})
            seed.update({k: float(mul[k]) for k in seed if k in mul})
            seed["f1_kneext_inh"] = 0.0
            print("seeded v5 winner into v6 (f1_kneext_inh 0)", flush=True)
    ...
    print("saved best_walk_params_v6.json (runner --best6 reproduces)",
          flush=True)
