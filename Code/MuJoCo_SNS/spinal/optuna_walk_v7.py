"""v6 study (ground_walk_v7_rom): v5 + swing-knee quad suppression.
...
Seeded with the v5 winner (best_walk_params_v6.json multipliers + the new
gain at 0). Writes best_walk_params_v7.json (runner --best6 loads it) and
...
STUDY = "ground_walk_v7_rom"
CSV = HERE / "v7_results.csv"
BEST6 = HERE / "best_walk_params_v7.json"
...
def full22_capture(study: optuna.Study, trial_no: int, p: dict):
    ...
    npz = HERE / f"v7_best_trial{trial_no}.npz"
...
def main(argv):
    ...
    if len(study.trials) == 0:
        ...
        try:
            prev = json.loads(
                (HERE / "best_walk_params_v6.json").read_text("utf-8"))
            mul = prev.get("multipliers", {})
            seed.update({k: float(mul[k]) for k in seed if k in mul})
            seed["f1_kneext_inh"] = 0.0
            print("seeded v6 winner into v7 (renshaw 0.5 fixed)", flush=True)
    ...
    print("saved best_walk_params_v7.json (runner --best7 reproduces)",
          flush=True)
