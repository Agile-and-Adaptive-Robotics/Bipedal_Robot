"""v6 study (ground_walk_v10_transient): v5 + swing-knee quad suppression.
...
gain at 0). Writes best_walk_params_v10.json (runner --best6 loads it) and
...
STUDY = "ground_walk_v10_transient"
CSV = HERE / "v10_results.csv"
BEST6 = HERE / "best_walk_params_v10.json"
...
def set_params(p: dict):
    ...
    params.G["ankle_post_walk_trim"] = float(p.get("ankle_post_walk_trim", 1.0))
...
def full22_capture(study: optuna.Study, trial_no: int, p: dict):
    ...
    npz = HERE / f"v10_best_trial{trial_no}.npz"
...
def objective(trial: optuna.Trial) -> float:
    p = dict(
        ...
        ankle_post_walk_trim=trial.suggest_float("ankle_post_walk_trim", 0.05, 1.0),
    )
    set_params(p)
    m = R.main(["--eval", "--drive", repr(p["drive"])])
    TRIAL_STATE[trial.number] = dict(params=p, metrics=m)
    # gates MUST sit below the worst genuine walker under this pose
    # (real slow gaits score -76..-86 here; -65/-80 sentinels would make
    # frozen models OUTSCORE walking - the v8b collapse, 2026-09-14)
    if m["nan"]:
...
def main(argv):
    ...
    if len(study.trials) == 0:
        ...
        try:
            ...
            print("seeded v9 winner into v10 (transient PRESET + ankle trim)", flush=True)
    ...
    eff = dict(drive=best.params["drive"],
               ...
               ankle_post_walk_trim=best.params["ankle_post_walk_trim"],
               renshaw=0.5)
    ...
    print("saved best_walk_params_v10.json (runner --best10 reproduces)",
          flush=True)
