...
## 8. Editor templates added (2026-09-25 evening)

Ben noticed the two variant networks were missing from the connectome block
editor's template library; both are now generated by
`spinal\make_editor_templates.py` (sidecar + embedded blob regenerated
together). Entries appear in the **SNS walker versions (winners)** dropdown
group:

| dropdown entry (TPL_JSON_NAMES) | library key | content |
|---|---|---|
| `w2lvar — W2L-layout s3k variant (2026-09-25)` | `w2lvar` | 980 nodes / 7526 edges = the 888 harvested neurons + 92 implied MUSCLE nodes; 7434 built synapses + 92 implied mn_to_muscle |
| `syn6 — 6-synergy walker (2026-09-25)` | `syn6` | 886 nodes / 3129 edges = 794 + 92 muscles; 3037 + 92 |

**Generation = the real networks, not a re-drawing.** Each template is
HARVESTED off the built/compiled SNS object (the `_net_edges.py` pattern:
every connection with conductance + reversal-potential sign), with the
stage-5 winner params applied through the authoritative loaders — the exact
`audit_replay_s1.py` merge (v10 multipliers shell + renshaw 0.5/0.0 pin +
syn6/syn6_brainstem pins + `curriculum_{variant}_stage5.json`) fed into
`_curriculum_{variant}.set_stage(5, p)`, then `build_network.build()`.
Params state is snapshotted/restored around each build (the curriculum
imports rescale the fitted W tables) and the variants are generated LAST so
the 16 pre-existing sidecar entries stay byte-identical. Scores/provenance
in each entry's `_note`; node `grp` layers use a variant-aware stamping
(the real net's lowercase `_r/_l` names: RG/InE/InF, PF cells + PF_IN,
V2a/V0V/V0D/V3E/INI, HEEL/TOE/TOEDF/LBIN, per-muscle motif INs, MN, RC,
afferents, muscles — 19 layers).

**Count note (documented deviation from the goal-4 builder-gate numbers):**
the templates carry the WINNER builds — 888/382/**7434** (w2lvar) and
794/382/**3037** (syn6), each +4 synapses over the gate-b default-param
counts (7430/3033) because the winners' searched `rg_weak_exc > 0` adds the
4 RG↔RG weak-excitation edges the default build lacks. The `_note` says so.

**Tests (all pass, run after the final regeneration):**
`node --check` on the HTML's last script block PASS; `_editor_template_test.js`
4/4 PASS; `_editor_edges_test.js` 17/17 PASS; `_editor_tree_test.js` 9/9
PASS; `_editor_static_check.py` clean (the `crumbBack` "missing id" is the
pre-existing v3.3 dynamic-innerHTML false positive); `_tpl_diff_groups_
20260924.py` **content diffs (excl grp): 0** over all 16 pre-existing
entries; `reports_20260925\tmp\gate_ab_w2lvar.py` GATE A PASS (defaults
still 410/376/1186) + GATE B PASS (888/382/7430 at default params).
No browser session was opened (per-origin localStorage caution); static
tests only.

**Two minimal test-file touch-ups (semantics unchanged):** (1) the regen
guard's `set(a) == set(b)` became `set(a) <= set(b)` — the library gained
keys, the guard still requires zero content diffs on the pre-existing set;
(2) its `%TEMP%` baseline was refreshed to the pre-w2lvar/syn6 sidecar
(the 2026-09-24 snapshot predated Ben's corrected rules/shevtsova/shinohara
exports of that night and could not reproduce — pre-existing staleness, not
this change). Note: the pre-change sidecar held **16** entries (the "21" in
earlier notes counted dropdown options incl. built-ins); it now holds 18.

**Regeneration command** (after any variant/builder change):
```
cd /d D:\Github\Bipedal_Robot\Code\MuJoCo_SNS\spinal
C:\Users\Ben Bolen\.conda\envs\myo\python.exe make_editor_templates.py
```
(writes `connectome_templates.json` AND re-embeds the blob between the
`/*TPL-EMBED*/` markers in `connectome_block_editor.html`; ~1–2 min — it
builds both variant networks; HTML is now ~1.5 MB, self-contained.)
