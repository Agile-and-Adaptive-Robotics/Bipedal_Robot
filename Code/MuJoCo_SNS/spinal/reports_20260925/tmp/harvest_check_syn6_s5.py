"""Harvest state check for curr_syn6_s5_walk_contact (stage 5, syn6)."""
import json
import os
import sqlite3
...
c_nan = c_frozen = c_real = 0
for n, st, v in vals:
    if v is None:
        continue
    if abs(v + 400.0) < 1e-9:
        c_nan += 1
    elif abs(v + 320.0) < 1e-9 or abs(v + 335.0) < 1e-9 or \
            abs(v + 345.0) < 1e-9 or abs(v + 325.0) < 1e-9:
        c_frozen += 1
    else:
        c_real += 1
print(f"floor census: -400(NaN)={c_nan} frozen/penalized-frozen={c_frozen} "
      f"real kine={c_real}")
con.close()
j = json.load(open(os.path.join(SP, "curriculum_syn6_stage5.json"),
                   encoding="utf-8"))
print("winner json: score", j["score"], "trial", j["trial"])
print("winner params:", json.dumps(j["params"]))
