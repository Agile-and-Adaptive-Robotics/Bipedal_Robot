"""v6 study (ground_walk_v8b_normal): v5 + swing-knee quad suppression.
...
gain at 0). Writes best_walk_params_v8b.json (runner --best6 loads it) and
...
STUDY = "ground_walk_v8b_normal"
CSV = HERE / "v8b_results.csv"
BEST6 = HERE / "best_walk_params_v8b.json"
...
def full22_capture(study: optuna.Study, trial_no: int, p: dict):
    ...
    npz = HERE / f"v8b_best_trial{trial_no}.npz"
...
def main(argv):
    ...
    if len(study.trials) == 0:
        ...
        try:
            prev = json.loads(
                (HERE / "best_walk_params_v8.json").read_text("utf-8"))
            mul = prev.get("multipliers", {})
            seed.update({k: float(mul[k]) for k in seed if k in mul})
            seed["f1_kneext_inh"] = 0.0
            print("seeded v8 hand-pose winner into v8b (normal.mot pose, 16 s eval)", flush=True)
    ...
    print("saved best_walk_params_v8b.json (runner --best8b reproduces)",
          flush=True)
